  ...
  <form @submit.prevent="submitForm">
    ...
    <div class="form-control" :class="{ invalid: !areas.isValid }">
      ...
      <div v-for="filter in filters" :key="filter">
        ...
        />
        <label :for="filter">{{ checkboxLabel(filter) }}</label>
      </div>
      <p v-if="!areas.isValid">
        At least one area of expertise must be checked
      </p>
    </div>
...
export default {
  ...
  methods: {
    ...
    checkboxLabel(filter) {
      return filter === filters.career
        ? `${this.capitalize(filter)} Advisory`
        : `${this.capitalize(filter)} Development`;
    },
  },
};
</script>
